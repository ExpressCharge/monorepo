...
This is an **aggregator-only** repo. It holds five submodules (`web`,
`email-worker`, `ios`, `steve`, `docs`) plus a top-level integrated dev
stack (Makefile + docker-compose.yml). No application code lives here.

## How to work in this repo

- **Don't commit code at the top level.** All real development happens
  inside the submodule directories (`web/`, `email-worker/`, `ios/`,
  `steve/`, `docs/`), which are independent git repositories with
  their own branches and CI.
...
| `docs`              | `cd docs && npm ci && npm run check && npm run build` |
...
- Deploy the docs site to Cloudflare Pages (lives in `docs/.github/workflows/deploy.yml`).
